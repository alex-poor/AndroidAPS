...
data class HovorkaParams(
    ...
    /**
     * Perturb THIS parameter set by the same clinically-plausible spreads [randomPatient] uses.
     *
     * WHY THIS EXISTS ALONGSIDE [randomPatient]. That builds a virtual patient from POPULATION constants,
     * which is right when the controller also runs the population model and badly wrong when the controller
     * is PERSONALISED from a profile: [personalize] can clamp insulin sensitivity to 0.2× population, so a
     * population-drawn plant can be ~7× more insulin-sensitive than the controller believes. A cohort built
     * that way is just a hypo generator — measured at 36% of time below range with 7% severe, and with every
     * arm of an A/B identical because the mismatch dwarfed the effect under test. Perturbing around the
     * controller's OWN physiology keeps the model error at a plausible size, which is the entire reason to
     * run a cohort instead of a single patient.
     */
    fun jitter(rng: java.util.Random, siFrac: Double = 0.40, egpFrac: Double = 0.20): HovorkaParams {
        fun j(frac: Double) = 1.0 + (rng.nextDouble() * 2 - 1) * frac
        val siMul = j(siFrac)
        return copy(
            f01 = f01 * j(egpFrac),
            egp0 = egp0 * j(egpFrac),
            kb1 = kb1 * siMul, kb2 = kb2 * siMul, kb3 = kb3 * siMul
        )
    }
    ...
    companion object {
        ...
        /**
         * Cohort insulin-absorption range (min, SC time-to-peak).
         *
         * WIDENED 2026-08-13 from 40–70 to 40–110 (report/hovorka-correction-rate-plan.md, Phase 4). The old
         * range described a settled cannula and nothing else. A day-1 site measures ~110 min (Hildebrandt
         * 1991), so every tuning decision ever taken against this cohort was taken against a plant whose
         * insulin ALWAYS arrives roughly on time — and against such a plant an optimiser will always prefer
         * faster, more decisive correction, because no state in the cohort punishes decisiveness. That is a
         * blind spot in the *test set*, not a bug in any one controller, and it is why it is fixed here rather
         * than worked around in a single harness.
         *
         * [TMAXI_COHORT_HI_NARROW] is retained so the historical cohort stays one argument away: passing it
         * reproduces any result recorded before this date exactly (the rng draw ORDER is unchanged, so a given
         * seed yields the same patient in every other parameter — narrow vs wide differ in tMaxI alone).
         */
        const val TMAXI_COHORT_LO = 40.0
        const val TMAXI_COHORT_HI = 110.0
        const val TMAXI_COHORT_HI_NARROW = 70.0    // pre-2026-08-13 cohort; settled sites only
         ...
         * plausible: insulin sensitivity ±40%, EGP/F01 ±20%, tMaxI 40–110, tMaxG 30–60, AG 0.7–0.9.
         */
        fun randomPatient(
            rng: java.util.Random, w: Double,
            tMaxIlo: Double = TMAXI_COHORT_LO, tMaxIhi: Double = TMAXI_COHORT_HI
        ): HovorkaParams {
            ...
            return HovorkaParams(
                ...
                ke = 0.138,
                vi = 0.12 * w,
                tMaxI = tMaxIlo + rng.nextDouble() * (tMaxIhi - tMaxIlo),
